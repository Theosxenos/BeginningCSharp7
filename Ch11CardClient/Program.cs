...
namespace Ch11CardClient
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            WriteLine($"Card.Equals({card3}, {card4}) ? { Card.Equals(card3, card4)}");
            WriteLine($"{card1} > {card2} ? {card1 > card2}");
            WriteLine($"{card1} <= {card3} ? {card1 <= card3}");
            WriteLine($"{card1} > {card4} ? {card1 > card4}");
            WriteLine($"{card4} > {card1} ? {card4 > card1}");
            WriteLine($"{card5} > {card4} ? {card5 > card4}");
            WriteLine($"{card4} > {card5} ? {card4 > card5}");
        }
    }
}
